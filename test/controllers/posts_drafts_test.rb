...
class PostsDraftsTest < ActionDispatch::IntegrationTest
  ...
  test "the index shows only published posts, with a drafts-only link" do
    ...
    assert_select "a[href=?]", drafts_path, text: "Edit your 1 draft…"
  ...
  test "scheduled-only wording" do
    ...
    get posts_path
    assert_select "a[href=?]", drafts_path, text: "Edit your 1 scheduled post"
  end

  test "combined wording when both exist" do
    schedule records(:typography)
    post posts_path, params: { post: { title: "Another draft" } }
    post posts_path, params: { post: { title: "Third draft" } }

    get posts_path
    assert_select "a[href=?]", drafts_path, text: "Edit your 1 scheduled post and 2 drafts…"
  end
  ...
  test "the drafts page lists drafts and scheduled posts, not published ones" do
    schedule records(:typography)
    post posts_path, params: { post: { title: "Fresh draft" } }
    ...
    assert_select "a.list__body[href=?]", edit_post_path(records(:typography))
