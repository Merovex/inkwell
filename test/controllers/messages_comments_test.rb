...
class MessagesCommentsTest < ActionDispatch::IntegrationTest
  ...
  test "the message page renders its thread with the prompt aimed at the message's composer" do
    ...
    assert_select "turbo-frame#new_comment a.comment__prompt[href=?]", new_message_comment_path(records(:welcome))
  end

  test "create originates a comment record parented to the message" do
    assert_difference -> { records(:welcome).comments.count } do
      post message_comments_path(records(:welcome)), params: { comment: { content: "<p>Welcome!</p>" } }
    end

    record = records(:welcome).children.order(:id).last
    assert_redirected_to message_path(records(:welcome), anchor: "comment_#{record.id}")
  end

  test "shallow comment actions resolve back to the message page" do
    patch comment_path(records(:welcome_comment)), params: { comment: { content: "<p>Edited reply.</p>" } }
    assert_redirected_to message_path(records(:welcome), anchor: "comment_#{records(:welcome_comment).id}")

    delete comment_path(records(:welcome_comment))
  ...
  test "the copy-link menu points at the message URL" do
    ...
    assert_select "[data-clipboard-text-value=?]",
      message_url(records(:welcome), anchor: "comment_#{records(:welcome_comment).id}")
  end
end
