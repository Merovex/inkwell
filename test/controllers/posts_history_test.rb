...
class PostsHistoryTest < ActionDispatch::IntegrationTest
  ...
  test "the six-line scenario: draft, publish, edit, revert, redraft, publish" do
    ...
    3.times { |i| patch admin_post_path(@record), params: { post: { title: "Draft #{i}", content: "<p>take #{i}</p>" } } }
    assert_equal 1, @record.versions.count

    post admin_post_publish_path(@record)                                    # publish
    patch admin_post_path(@record), params: { post: { title: "Draft 2", content: "<p>tightened</p>" } } # tracked edit
    delete admin_post_publish_path(@record)                                  # revert to draft
    4.times { |i| patch admin_post_path(@record), params: { post: { content: "<p>redraft #{i}</p>", title: "Draft 2" } } }
    post admin_post_publish_path(@record)                                    # publish again

    assert_equal 5, @record.versions.count, "created, published, updated, unpublished, published"

    get admin_post_events_path(@record)
  ...
  test "a title-only tracked change renders the from/to line" do
    ...
    patch admin_post_path(record), params: { post: { title: "New title" } }
  ...
  test "See the changes links past versions that differ from current" do
    ...
    patch admin_post_path(record), params: { post: { title: posts(:kickoff).title, content: "<p>Pulling together the themes we settled on last week.</p>" } }

    get admin_post_events_path(record)
    assert_response :success
    assert_select "a.history__action[href=?]", admin_post_change_path(record, v1), text: "See the changes"
    # The current version never links to a self-diff.
    assert_select "a[href=?]", admin_post_change_path(record, record.reload.recordable), count: 0
  end
  ...
  test "the change page diffs the past version against the CURRENT one, not its neighbor" do
    ...
    v1 = record.recordable
    patch admin_post_path(record), params: { post: { title: posts(:kickoff).title, content: "<p>Pulling together the themes we settled on last week.</p>" } }
    patch admin_post_path(record), params: { post: { title: posts(:kickoff).title, content: "<p>Pulling together the themes we settled on last month.</p>" } }

    get admin_post_change_path(record, v1)
    assert_response :success
  ...
  test "a frozen version renders read-only" do
    record = records(:kickoff)
    v1 = record.recordable
    patch admin_post_path(record), params: { post: { title: "Renamed", content: "<p>changed</p>" } }

    get admin_post_version_path(record, v1)
    assert_response :success
