...
class CommentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "post page renders the thread and the comment prompt, no composer" do
    ...
    assert_select "turbo-frame#new_comment a.comment__prompt[href=?]", new_admin_post_comment_path(records(:kickoff))
    assert_select "#new_comment lexxy-editor", count: 0
  end

  test "new swaps the composer into the prompt's frame" do
    get new_admin_post_comment_path(records(:kickoff))
    assert_response :success
    assert_select "turbo-frame#new_comment lexxy-editor"
    assert_select "turbo-frame#new_comment form[action=?]", admin_post_comments_path(records(:kickoff))
  ...
  test "create originates a comment record parented to the post" do
    assert_difference -> { records(:kickoff).comments.count } do
      post admin_post_comments_path(records(:kickoff)), params: { comment: { content: "<p>Nice.</p>" } }
    end

    record = records(:kickoff).children.order(:id).last
    assert_redirected_to admin_post_path(records(:kickoff), anchor: "comment_#{record.id}")
    assert_equal users(:alice), record.creator
  end

  test "create rejects a blank comment" do
    assert_no_difference -> { Comment.count } do
      post admin_post_comments_path(records(:kickoff)), params: { comment: { content: "" } }
    end
    assert_redirected_to admin_post_path(records(:kickoff), anchor: "new_comment")
  end

  test "edit renders the inline editor in the comment's frame" do
    get edit_admin_comment_path(records(:kickoff_comment))
    assert_response :success
  ...
  test "update lands as a tracked version and shows the Edited marker" do
    ...
    assert_difference -> { record.versions.count } do
      patch admin_comment_path(record), params: { comment: { content: "<p>Better.</p>" } }
    end
  ...
  test "update with blank content re-renders the editor" do
    patch admin_comment_path(records(:kickoff_comment)), params: { comment: { content: "" } }
    assert_response :unprocessable_entity
    assert_select "lexxy-editor"
  end

  test "destroy trashes the comment" do
    delete admin_comment_path(records(:kickoff_comment))
  ...
  test "member actions are yours-only: someone else's comment 404s" do
    ...
    patch admin_comment_path(records(:kickoff_comment)), params: { comment: { content: "<p>Hijacked.</p>" } }
    assert_response :not_found
