...
class UserSettingsTest < ActionDispatch::IntegrationTest
  ...
  test "show renders the name form and the avatar well" do
    ...
    assert_select "form[action=?][enctype=?]", user_avatar_path, "multipart/form-data"
  ...
  test "update changes the display name" do
    patch user_settings_path, params: { user: { name: "Alice B. Example" } }
  ...
  test "uploading a picture attaches it and shows it in the avatar" do
    patch user_avatar_path, params: { user: { avatar: fixture_file_upload("avatar.png", "image/png") } }
  ...
  test "rejects a non-image upload" do
    patch user_avatar_path, params: { user: { avatar: fixture_file_upload("avatar.txt", "text/plain") } }
