...
class SuppressionTest < ActiveSupport::TestCase
  ...
  test "a hard bounce suppresses the person for every site" do
    ...
    assert @person.suppressed_for?(@site)
    assert @person.suppressed_for?(@other)
  end
  ...
  test "a complaint suppresses only the site that sent the mail" do
    ...
    assert_not @person.suppressed?
    assert @person.suppressed_for?(@site)
    assert_not @person.suppressed_for?(@other)
  end
  ...
  test "complaints against ESCALATE_AFTER distinct sites go global" do
    ...
    assert @person.suppressed?, "two sites complained about"
  ...
  test "a site-scoped lift of a global suppression frees that site alone" do
    ...
    assert @person.suppressed_for?(@other), "the other site is still bound by the global row"
    assert @person.suppressed?, "the global list itself is untouched"
  end
