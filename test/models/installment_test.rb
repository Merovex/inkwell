...
class InstallmentTest < ActiveSupport::TestCase
  test "a series and book in the same account pair up" do
    ...
    assert Installment.new(series_record: series_record, book_record: book_record).valid?
  end
  ...
  test "a cross-account pairing is invalid" do
    ...
    installment = Installment.new(series_record: series_record, book_record: book_record.reload)
