...
class Exporter
  ...
  private
    ...
    # Bodies leave through Exporter::Prose (shared with the author's archive):
    # inline images are copied into the workspace like any cover or logo —
    # blob id in the prefix so two photos sharing a filename can't overwrite
    # each other — and stamped with their measured size from image_sizes.
    def html(rich_text)
      @prose ||= Exporter::Prose.new(sizes: image_sizes) { |blob| copy_blob(blob, "inline-#{blob.id}") }
      @prose.html(rich_text)
    end
