...
module CommentsHelper
  def commentable_path(record, **options)
    public_send(:"#{record.recordable_name}_path", record, **options)
  end

  def commentable_url(record, **options)
    public_send(:"#{record.recordable_name}_url", record, **options)
  end

  def commentable_comments_path(record)
    public_send(:"#{record.recordable_name}_comments_path", record)
  end

  def new_commentable_comment_path(record)
    public_send(:"new_#{record.recordable_name}_comment_path", record)
  end
end
