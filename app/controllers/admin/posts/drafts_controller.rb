...
class Admin::Posts::DraftsController < ApplicationController
  ...
  def destroy
    if @post.published?
      redirect_to admin_drafts_path, alert: "Published posts go to the trash, not the shredder."
    elsif @post.ever_published?
      @record.trash
      redirect_to admin_drafts_path, notice: "Moved to the trash — once-published posts are retained."
    else
      @record.destroy
      redirect_to admin_drafts_path, notice: "Draft deleted."
    end
  end
end
