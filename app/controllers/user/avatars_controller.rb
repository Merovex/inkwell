...
class User::AvatarsController < ApplicationController
  def update
    if Current.user.update(avatar_params)
      redirect_to user_settings_path, notice: "Picture updated."
    else
      redirect_to user_settings_path, alert: Current.user.errors.full_messages.to_sentence
    end
  end

  def destroy
    Current.user.avatar.purge_later
    redirect_to user_settings_path, notice: "Picture removed — using your initials."
  end
